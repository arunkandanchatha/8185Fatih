...
!
    ...
    REAL(DP):: AV, AVSQ, C, DDX, DDY, DMIN
    REAL(DP):: DTOL
    REAL(DP):: DX, DY, FK, RMAX, RQ, RS, RSMX, RSOLD
    REAL(DP), PARAMETER :: RTOL = 1.0E-05_dp
    REAL(DP):: RWS, S
    REAL(DP), PARAMETER :: SF = 1.0E+00_dp
    REAL(DP):: SUM2, T, XK, XMIN, XMN, YK, YMIN, YMN
    REAL(DP), DIMENSION(N) :: F, RSQ, X, Y
    REAL(DP), DIMENSION(5,N) :: A
    REAL(DP), DIMENSION(6,6) :: B

    DTOL = SQRT(EPSILON(1.0_dp))
    NN = N
    NNR = NR
    NQWMAX = MAX (NQ,NW)
    LMAX = MIN (40,N-1)
...
!
                IF (RWS == 0.0E+00_dp) THEN
                    RWS = 1.1E+00_dp * RS
                END IF

                IF (RQ == 0.0E+00_dp) THEN
                    NEQ = LMAX
                    RQ = SQRT (1.1E+00_dp * RS)
                    AVSQ = SUM2 / REAL(NEQ, DP)
                END IF
!
...
!
            ...
            IF (RWS == 0.0E+00_dp .AND. LNP > NW) THEN
                RWS = RS
            END IF
!
...
! 5 <= NQ <= NEQ < LMAX <= N-1.
!
            IF (RQ == 0.0E+00_dp .AND. LNP > NQ) THEN
                NEQ = LNP - 1
                RQ = SQRT (RS)
                AVSQ = SUM2 / REAL (NEQ, DP)
            END IF
...
!
    ...
    REAL(DP):: DELX, DELY, DS, DX, DY, PX, PY, QS2VAL, RD, RDS, &
    ...
    ELSE IF ( NR < 1  ) THEN
        RETURN
    ELSE IF ( DX <= 0.0E+00_dp ) THEN
        RETURN
    ELSE IF ( DY <= 0.0E+00_dp ) THEN
        RETURN
    ELSE IF ( RMAX < 0.0E+00_dp ) THEN
        RETURN
    END IF
!
...
!
    ...
    REAL(DP) :: DELX, DELY, DS, DX, DY, PX, PY, Q, QK, QKX, &
            ...
            IF (K /= 0) THEN
                EVAL: DO
                    ...
                    IF (DS < RS) THEN
                        ...
                        RD = SQRT (RDS)
                        W = (RS + DS - RD - RD) / RDS
                        T = 2.0E+00_dp * (RD - RS) / (DS * RDS)
                        WX = DELX * T
                        WY = DELY * T
                        QKX = 2.0E+00_dp * A(1,K) * DELX + A(2,K) * DELY
                        QKY = A(2,K) * DELX + 2.0E+00_dp * A(3,K) * DELY
                        QK = (QKX * DELX + QKY * DELY) / 2.0E+00_dp
                        QKX = QKX + A(4,K)
                        QKY = QKY + A(5,K)
...
!
    ...
    REAL(DP) :: DELX, DELY, DSQ, DX, DY, PX, PY, R, RSMIN, RSQ, &
          XMIN, XP, YMIN, YP
...
!
    IF (NR < 1 .OR. DX <= 0.0E+00_dp .OR. DY <= 0.0E+00_dp) THEN
        NP = 0
        DSQ = 0.0E+00_dp
    END IF
!
...
!
    ...
    REAL(DP):: D, DX, DY, FI, FK, R, S1, S2, W, XI, YI, XK, YK
...
!
    ...
    REAL(DP) :: DX, DY, XMAX, XMIN, YMAX, YMIN
    REAL(DP), DIMENSION(N) :: X, Y
...
!
    DX = (XMAX - XMIN) / REAL (NR, DP)
    DY = (YMAX - YMIN) / REAL (NR, DP)
!
! Test for zero area.
!
    IF ( DX == 0.0E+00_dp .OR. DY == 0.0E+00_dp ) THEN
        IER = 2
        RETURN
    END IF
!
